...
async function findTickets({ status, priority, category, assignedTo, createdBy, page = 1, limit = 20 } = {}) {
  ...
  const query = `SELECT META().id as id, t.* FROM \`travel-sample\`._default.tickets t WHERE ${where.join(' AND ')} ORDER BY t.createdAt DESC LIMIT $${params.length + 1} OFFSET $${params.length + 2}`;
  params.push(limit, offset);

  const result = await cluster.query(query, { parameters: params });
  const rows = await result.rows;

  const countQuery = `SELECT COUNT(*) as total FROM \`travel-sample\`._default.tickets t WHERE ${where.join(' AND ')}`;
...
async function getStats() {
  ...
  const statusQuery = `SELECT t.status, COUNT(*) as count FROM \`travel-sample\`._default.tickets t WHERE t.type = 'ticket' GROUP BY t.status`;
  const priorityQuery = `SELECT t.priority, COUNT(*) as count FROM \`travel-sample\`._default.tickets t WHERE t.type = 'ticket' GROUP BY t.priority`;
  const categoryQuery = `SELECT t.category, COUNT(*) as count FROM \`travel-sample\`._default.tickets t WHERE t.type = 'ticket' GROUP BY t.category`;
  const totalQuery = `SELECT COUNT(*) as total FROM \`travel-sample\`._default.tickets t WHERE t.type = 'ticket'`;
  const recentQuery = `SELECT META().id as id, t.* FROM \`travel-sample\`._default.tickets t WHERE t.type = 'ticket' ORDER BY t.createdAt DESC LIMIT 5`;
